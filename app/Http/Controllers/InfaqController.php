...
class InfaqController extends Controller
{
    public function index(Request $request)
    {
        if (isset($_GET['search'])) {
            $search = $_GET['search'];
            $infaq = Infaq::where('title', 'like', '%' . $search . '%')->paginate(10);
        } else if (isset($_GET['start_date']) && isset($_GET['end_date'])) {
            $start_date = $_GET['start_date'];
            $end_date = $_GET['end_date'];
            $infaq = Infaq::whereBetween('date', [$start_date, $end_date])->paginate(10);
        } else if (isset($_GET['start_date'])) {
            $start_date = $_GET['start_date'];
            $infaq = Infaq::where('date', '>=', $start_date)->paginate(10);
        } else if (isset($_GET['end_date'])) {
            $end_date = $_GET['end_date'];
            $infaq = Infaq::where('date', '<=', $end_date)->paginate(10);
        } else if (isset($_GET['verifier_id'])) {
            $verifier_id = $_GET['verifier_id'];
            $infaq = Infaq::where('verifier_id', $verifier_id)->paginate(10);
        } else if (isset($_GET['amount'])) {
            $amount = $_GET['amount'];
            $infaq = Infaq::where('amount', $amount)->paginate(10);
        } else if (isset($_GET['amount_min']) && isset($_GET['amount_max'])) {
            $amount_min = $_GET['amount_min'];
            $amount_max = $_GET['amount_max'];
            $infaq = Infaq::whereBetween('amount', [$amount_min, $amount_max])->paginate(10);
        } else if (isset($_GET['amount_min'])) {
            $amount_min = $_GET['amount_min'];
            $infaq = Infaq::where('amount', '>=', $amount_min)->paginate(10);
        } else if (isset($_GET['amount_max'])) {
            $amount_max = $_GET['amount_max'];
            $infaq = Infaq::where('amount', '<=', $amount_max)->paginate(10);
        } else if (isset($_GET['sort'])) {
            $sort = $_GET['sort'];
            $infaq = Infaq::orderBy('amount', $sort)->paginate(10);
        } else if (isset($_GET['sort_date'])) {
            $sort_date = $_GET['sort_date'];
            $infaq = Infaq::orderBy('date', $sort_date)->paginate(10);
        } else if (isset($_GET['sort_verifier'])) {
